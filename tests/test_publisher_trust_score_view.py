...
class TestPublisherTrustScoreView:
    def test_view_computes_weighted_24h_trust_scores(self, db_session):
        ...
        db_session.execute(
            text(
                ...
                VALUES
                    ...
                    -- Excluded from view window (>24h old)
                    ...
                    -- Publisher C has only old data, so should not appear in the view
        ...
        rows = db_session.execute(
            text(
                """
                SELECT publisher_id, trust_score::double precision AS trust_score, last_calculated
        ...
        assert [r["publisher_id"] for r in rows] == [PUB_A, PUB_B]
        ...
        expected_b = _expected_trust([(0.70, 2.0)])
        ...
        # last_calculated should be the most recent in-window model_logs timestamp.
        ...
        assert by_publisher[PUB_A]["last_calculated"] == latest_by_pub[PUB_A]
        assert by_publisher[PUB_B]["last_calculated"] == latest_by_pub[PUB_B]
